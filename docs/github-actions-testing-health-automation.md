...
- `GITHUB_TOKEN` for Copilot requests and pull request operations where your account and repository settings support it.
- A pull request for reviewable updates to `docs/testing-framework-health.csv` and `docs/testing-framework-health.md`.
- Optional `HEALTH_PR_TOKEN` if repository settings do not allow `GITHUB_TOKEN` to create pull requests.
...
5. Enable Allow GitHub Actions to create and approve pull requests. Without this setting, `gh pr create` fails with `GitHub Actions is not permitted to create or approve pull requests` even when the workflow has `pull-requests: write`.
6. If that setting is unavailable or you prefer not to enable it, create a fine-grained personal access token with Contents: Read and write and Pull requests: Read and write for this repository, then store it as the repository secret `HEALTH_PR_TOKEN`.
7. Go to your GitHub profile settings and confirm GitHub Copilot Free is enabled for your account.
8. Commit and push these files to `main`:
...
10. Select the Testing Framework Health workflow.
11. Click Run workflow.
12. Review the pull request created by the workflow.
13. Merge it manually if auto-merge is blocked by repository settings.
...
- The workflow requires `pull-requests: write` plus the repository-level Allow GitHub Actions to create and approve pull requests setting when it uses `GITHUB_TOKEN` for PR creation.
